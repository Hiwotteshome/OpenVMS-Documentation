...
int main() {
    ...
    recvAddr.sin_addr.s_addr = INADDR_ANY; // Listen on all available interfaces
    recvAddr.sin_port = htons(12345);     // Listen on port 12345
    ...
    int bytesReceived = recvfrom(
        recvSocket,
        recvBuf,
        sizeof(recvBuf) - 1,  // leave space for null terminator
        0,
        (SOCKADDR*)&senderAddr,
        &senderAddrSize
    );
